...
public class EAGLE extends Algorithm{
     ...
     */
    public List<Cluster>  EAGLEFinder(CyNetwork inputNetwork,int resultTitle){
...
/*        Cluster[] clusters = new Cluster[alClusters.size()];
    ...
    private void mergeComplex(ArrayList complexes)//the number of complexes must be no less than two
    {
    	...
    	List<Long> nodes1=((Cluster)complexes.get(index1)).getALCluster();
    	List<Long> nodes2=((Cluster)complexes.get(index2)).getALCluster();   
