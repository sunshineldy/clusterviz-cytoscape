...
/**
 * A loader for loading picture with a progress bar 
 * for the complex browser table
 */
public class Loader extends ImageIcon implements Runnable {
    ...
    /**
     * Generates a loading picture with a progress bar for the complex browser table
     *
    ...
    Loader(int selectedRow, JTable table, int width, int height)
    {
      this.selectedRow = selectedRow;
      this.table = table;

      this.bgColor = table.getSelectionBackground();
      this.fgColor = table.getSelectionForeground();
      this.fontSize = 8;

      this.onScreenLoader = new BufferedImage(width, height, 2);
      this.onScreenG2 = ((Graphics2D)this.onScreenLoader.getGraphics());

      this.offScreenLoader = new BufferedImage(width, height, 2);
    ...
    Loader (JTable table, int width, int height) {
        this.table = table;
        ...
        //We create the on screen image which will always contain
        //a completed picture for the table to paint whenever it can
        ...
        //We also create the off screen image which we will use to compute the image
        //and eventually transfer the completed image to the onscreen image
        ...
        offScreenG2.setFont(new Font("Times New Roman", Font.PLAIN, fontSize));
     ...
     * with progress set to 0 and process to "Starting".
     *
            ...
            while (true) {
                if (loading) {
                    ...
                    if (loading) {
                        ...
                        //This paintImmediately function causes problems in windows 
                        //(paints on every component sometimes)
                        //Rectangle bounds = table.getCellRect(selectedRow, 0, false);
                        //if (loading) table.paintImmediately(bounds);
                        //This fireTableCellUpdated function seems to consolidate some painting too 
                        //so it is not very responsive in windows
