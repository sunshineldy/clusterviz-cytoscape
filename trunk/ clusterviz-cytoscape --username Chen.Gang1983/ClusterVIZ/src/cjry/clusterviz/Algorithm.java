...
public class Algorithm {
    private boolean cancelled = false;//If set, will schedule the cancelled algorithm  at the next convenient opportunity
    ...
    private TreeMap<Double,ArrayList> curNodeScores = null; //key is node score, value is nodeIndex
    private TreeMap<Double,ArrayList> curEdgeWeights=null;	//key is edge index,value is EdgeInfo instance
    private HashMap<Integer,Clique> curCliques=null;		//key is clique ID,value is Clique instance
    private ArrayList<Cluster> curOptimalDivision=null;
    ...
    private HashMap<String,HashMap> maximalCliquesNetworkMap=new HashMap<String,HashMap>();	//key is networkID, value is maximal Cliques
    private HashMap<String,TreeMap> edgeWeightNetworkMap=new HashMap<String,TreeMap>();
    //private HashMap<String,HashMap> nodeInfoNetworkMap=new HashMap<String,HashMap>();
    private HashMap<String,ArrayList> optimalDivisionKeyMap=new HashMap<String,ArrayList>();
    ...
    private class NodeInfo {
        ...
        int iComplex;
        ArrayList<Integer> alComplex=new ArrayList<Integer>();
        public NodeInfo() {
            this.density = 0.0;
            this.coreLevel = 0;
		...
		public ArrayList<Integer> getAlComplex() {
			return alComplex;
		}
		public void setAlComplex(ArrayList<Integer> alComplex) {
    ...
    public void calEdgeWeight(CyNetwork inputNetwork){
    	...
    	if(!edgeWeightNetworkMap.containsKey(networkID)){
        	...
        	TreeMap<Double,ArrayList> edgeWeightsMap=new TreeMap<Double,ArrayList>(new Comparator(){
        	...
        	while(edges.hasNext()&&!cancelled){
        		...
        		for(int i=0;i<neighbors2.length;i++){
        			...
        			if(Arrays.binarySearch(neighbors1, key)>=0)
        				common++;//exist a common neighbor of both nodes
        		}
        		common++;
        		...
        		if(edgeWeightsMap.containsKey(new Double(weight))) {
        			al=(ArrayList<Integer>)edgeWeightsMap.get(new Double(weight));
        			al.add(e.getRootGraphIndex());
        		}else{
    	...
    	/*System.out.println("\nThe edge weights:");
    	TreeMap map=curEdgeWeights;
    	for(Iterator i1=map.keySet().iterator();i1.hasNext();){
    		Double w=(Double)i1.next();
    		System.out.println("Value"+w.doubleValue());
    		ArrayList als=(ArrayList)map.get(w);
    		for(Iterator i2=als.iterator();i2.hasNext();){
    			int e=((Integer)i2.next()).intValue();
   ...
   public Cluster[] FAG_ECFinder(CyNetwork inputNetwork,String resultTitle){
        ...
        ArrayList<Cluster> alOriginalClusters = new ArrayList<Cluster>(inputNetwork.getNodeCount());
        ...
        System.out.println("Initially NumComplex: "+alOriginalClusters.size());
        /**********************************************************************************************
			Then, Operation UNION:	according to different situation, in which the two nodes consisting 
				this arc may belong to differnt Complexes or an identicial Complex and that the 
        ...
        for (Iterator iterator = values.iterator(); iterator.hasNext();) {
            ...
            for (int j = 0; j < alEdgeWithSameWeight.size(); j++) {//for each edge
        		...
        		NodeInfo fromNI=(NodeInfo)curNodeInfos.get(new Integer(inFrom));	//起始节点Info
        		NodeInfo toNI=(NodeInfo)curNodeInfos.get(new Integer(inTo));	//终端节点Info
        		
        		int icFrom=fromNI.iComplex;	//起始节点所属Complex
        		int icTo=toNI.iComplex;		//终端节点所属Complex  
        		if(icFrom != icTo)    //相同就不做处理
        		{
        			Cluster cFrom=(Cluster)alOriginalClusters.get(icFrom);
        			Cluster cTo=(Cluster)alOriginalClusters.get(icTo);
        			if(cFrom.isMergeable() && cTo.isMergeable())	//两Complex都可合并
        				if(!cFrom.isModule() || !cTo.isModule())	//两Complex不都是模块
        					if(cFrom.getALNodes().size() >= cTo.getALNodes().size()){//【 将较小的Complex。。。
        				...
        				else	//至少有一个Complex属于功能模块，则两个Complex都标记为不可合并的
        				{
        					cFrom.setMergeable(false);
        					cTo.setMergeable(false);
        				}
        			else	//至少有一个Complex是不可合并的，则两个Complex都标记为不可合并的
        			{
        				cFrom.setMergeable(false);
    					cTo.setMergeable(false);
        			}
        ...
        ArrayList<Cluster> alClusters = new ArrayList<Cluster>();
        ...
        while(it.hasNext()){
        	...
        	if(cluster.getALNodes().size()>=params.getComplexSizeThreshold()){
        		...
        		cluster.setSeedNode(((Integer)alNodes.get(0)).intValue());
        ...
        /*System.out.println("\tThere are "+alClusters.size()+" clusters in total");
        Iterator it1=alClusters.iterator();
        while(it1.hasNext()){
        	Cluster cl=(Cluster)it1.next();
        	ArrayList alNodes=cl.getALNodes();
        	Iterator it2=alNodes.iterator();
        	System.out.println("Cluster "+cl.getComplexID()+" Size "+alNodes.size());
        	while(it2.hasNext()){
        		int inde=((Integer)it2.next()).intValue();
        		String name=inputNetwork.getNode(inde).getIdentifier();
    ...
    public void getMaximalCliques(CyNetwork inputNetwork,String resultTitle){
        String callerID = "Algorithm.getCliques";
    	...
    	if(!maximalCliquesNetworkMap.containsKey(inputNetwork.getIdentifier())){
            ...
            long msTimeBefore = System.currentTimeMillis();
    		HashMap<Integer,Clique> cliques = new HashMap<Integer,Clique>();
    		
        	//initialize states
    		Vector<Integer> alCur=new Vector<Integer>();
    		Vector<Integer> alFini=new Vector<Integer>();
    		Vector<Integer> alNodes=new Vector<Integer>(inputNetwork.getNodeCount());
    		for(Iterator i=inputNetwork.nodesIterator();i.hasNext();){
    			Integer node=new Integer(((Node)i.next()).getRootGraphIndex());
    			alNodes.add(node);
    		}    		
    		//The critical interal process
    	...
    	/*System.out.println("The maximal Cliques:");
    	for(Iterator i=curCliques.keySet().iterator();i.hasNext();){
    		Integer id=(Integer)i.next();
    		Clique cur=(Clique)curCliques.get(id);
    		System.out.println("Clique"+id.intValue()+"\tCliqueID is "+cur.getCliuqueID());
    		ArrayList nodes=cur.getCliqueNodes();
    		for(Iterator it=nodes.iterator();it.hasNext();){
    			int in=((Integer)it.next()).intValue();
    			String name=currentNetwork.getNode(in).getIdentifier();
    			System.out.print(name+" ");
    		}
    		System.out.println();
    	}*/    	
     ...
     */
    private void expand(HashMap<Integer,Clique> cliques,Vector<Integer> curr, Vector<Integer> cand, 
    		Vector<Integer> not){
    	...
    	else
    	{
    		int p,i;
    		Integer q;
    		Vector<Integer> candq;
    		Vector<Integer> notq;
    		Vector<Integer> cutcand=new Vector((Vector)cand.clone());
    		p=getPivot(cand,not);	//get the index of the pivot node 
    		ArrayList<Integer> cuts=getNeighbors(p);
    		for(Iterator it=cuts.iterator();it.hasNext();)//get the trimed candidates
    		...
    		for(i=0;i<cutcand.size();++i)//for each non-adjacent node
    		{
    			...
    			ArrayList<Integer> adjs=getNeighbors(q);//1.2 get the adjacents
    			candq=getIntersect( cand,adjs );//2.1 get inserction
    			notq=getIntersect(not,adjs);
    			expand(cliques,curr,candq,notq);//2.3 incursive invoking
    			not.add(q);
    ...
    //Choose a vertex from cand&not with the largest number of 
    //connections to the other vertices in CANDIDATES
    private int getPivot(Vector<Integer> cand, Vector<Integer> not){
    ...
    private Vector<Integer> getIntersect(Vector<Integer> a,ArrayList<Integer> b){
   ...
   /****************First, we sort each maximal clique into a complex**************/
        ArrayList<Cluster> rawClusters=new ArrayList<Cluster>();
        ...
        for(int i=0;i<curCliques.size();i++){	//for each clique
            ...
            ArrayList<Integer> nodes=cur.getCliqueNodes();
        	...
        	if(cur.isSubordinate()){
        		...
        		while(it.hasNext()){
            		...
            		if(searchInComplexes(rawClusters,n)==0){	//a subordinate node
            			...
            			newCluster.setTotalDegree(totalDegree);
                		((NodeInfo)curNodeInfos.get(n)).getAlComplex().add(counter++);
                    	rawClusters.add(newCluster); 
            		}
        		}
        	}
        }//completed initialization
        /*System.out.println("After initializing maximal cliques into complexes"+
        		"\nThere are "+rawClusters.size()+" rawComplexes");
        for(Iterator it1=rawClusters.iterator();it1.hasNext();){
        	Cluster cur=(Cluster)it1.next();
        	ArrayList nodes=cur.getALNodes();
        	System.out.println("Com"+(cur.getComplexID()+1)+":  "+cur.isModule());
        	for(Iterator it2=nodes.iterator();it2.hasNext();){
        		int index=((Integer)it2.next()).intValue();
        		String name=inputNetwork.getNode(index).getIdentifier();
                System.out.print(name+" ");
        	}
        	System.out.println();
        }*/
     ...
     ***********************************************************************************************/
        ...
        for (Iterator iterator = values.iterator(); iterator.hasNext();) {//for each weight values
            ...
            for (int j = 0; j < alEdgeWithSameWeight.size(); j++) {//for each edge
        		...
        		ArrayList<Integer> subFrom=fromNI.getAlComplex();	//起始节点所属Complex
        		ArrayList<Integer> subTo=toNI.getAlComplex();		//终端节点所属Complex
        		if(different(subFrom,subTo))    //相同就不做处理
        		{
        			//System.out.println("\nThe arc: "+curEdge.getSource().getIdentifier()+"->"+curEdge.getTarget().getIdentifier());
        			Cluster c1,c2,c;
        			//merge all possible complexes
    				c1=rawClusters.get( ((Integer)subFrom.get(0)).intValue() );
    				if(subFrom.size()>1){
    	    			while(true){
    	    				int other=subFrom.indexOf(new Integer(c1.getComplexID()))+1;
    	    				if(cancelled||(other>=subFrom.size()))	
    	    					break;
    	    				c=rawClusters.get(((Integer)subFrom.get(other)).intValue());
    				...
    				//subTo=toNI.getAlComplex();
    				c2=rawClusters.get( ((Integer)subTo.get(0)).intValue() );
    				if(subTo.size()>1 && different(subFrom,subTo)){
    	    			while(true){
    	    				int other=subTo.indexOf(new Integer(c2.getComplexID()))+1;
    	    				if(cancelled||(other>=subTo.size()))	
    	    					break;
    	    				c=rawClusters.get(((Integer)subTo.get(other)).intValue());
    	    				    	    				/*System.out.print("\nthe subTo: ");
    	    				for(Iterator i1=subTo.iterator();i1.hasNext();){
    	    					System.out.print(i1.next().toString()+" ");
    	    				}
    	    				System.out.println();*/
	    					if(!c2.isModule() && !c.isModule())
	    						...
	    						//System.out.print("C"+c2.getComplexID()+" has forged a module");
		    					c2.setMergeable(false);
		    					c2=c;
		    				}    	    				
    	    			}
    				}
    				if(c1.getComplexID()!=c2.getComplexID()){
	    				//System.out.print("\nAt last merge: "+c1.getComplexID()+" "+c2.getComplexID());
        ...
        ArrayList<Cluster> alClusters=new ArrayList<Cluster>();
        //System.out.println("\n\nThe non-empty elements in rawClusters:");
        ...
        while(it1.hasNext()){
        	...
        	if(cluster.getALNodes().size()>=params.getComplexSizeThreshold()){
        		...
        		if(ind!=0 && outd!=0)
            		...
            		cluster.calModularity(inputNetwork);
        		alClusters.add(cluster);
        		/*Iterator ir=alNodes.iterator();
        		while(ir.hasNext()){
        			int index=((Integer)ir.next()).intValue();
        			System.out.print(inputNetwork.getNode(index).getIdentifier()+" ");
        		}
        		System.out.println();*/
        	}
        }
    ...
    private boolean different(ArrayList<Integer> a, ArrayList<Integer> b){
    ...
    public Cluster[] EAGLEFinder(CyNetwork inputNetwork,String resultTitle){
       ...
       /********************Initialize the complexes*************************/
        	ArrayList<Cluster> alClusters=new ArrayList<Cluster>();
            ArrayList<Cluster> optimals=null;
            for(int i=0;i<curCliques.size();i++){
                Clique cur=(Clique)curCliques.get(new Integer(i));
                ArrayList<Integer> nodes=cur.getCliqueNodes();
            ...
            do{
        		...
        		if(max==0 || cur>max){
        			...
        			optimals=temp;
        			//optimal=new ArrayList<Cluster>((ArrayList)alClusters.clone());
        		}
                if (taskMonitor != null) {
                    findingProgress++;
                    int newProgress = (findingProgress * 100) / findingTotal;
     ...
     */
    private void mergeComplex(ArrayList<Cluster> complexes)//the number of complexes must be no less than two
    {
    	...
    	ArrayList<Integer> nodes1=complexes.get(index1).getALNodes();
    	ArrayList<Integer> nodes2=complexes.get(index2).getALNodes();    	
        /*System.out.println("The two merged Complexes:"+index1+" and "+index2);
    	for(Iterator i1=nodes1.iterator();i1.hasNext();){
    		int in=((Integer)i1.next()).intValue();
    		String name=currentNetwork.getNode(in).getIdentifier();
    		System.out.print(name+" ");
    	}
		System.out.println();
    	for(Iterator i2=nodes2.iterator();i2.hasNext();){
    		int in=((Integer)i2.next()).intValue();
    		String name=currentNetwork.getNode(in).getIdentifier();
    		System.out.print(name+" ");
    	}
    	System.out.println();*/    	
    ...
    /**
     * calculate the simiarities between a pairs of complexes
     ...
     */
    private double calModularity(ArrayList<Cluster> alClusters){
    	double M=0.0,temp;
    	int A,i,j,n1,n2;
    	int d1,d2,c1,c2;
    	int[] neighs;
    ...
    private int searchInComplexes(ArrayList<Cluster> alClusters, Integer node){
    	int counter=0;
    ...
    public void scoreGraph(CyNetwork inputNetwork, String resultTitle) {
        ...
        long msTimeBefore = System.currentTimeMillis();
        HashMap<Integer,NodeInfo> nodeInfoHashMap = new HashMap<Integer,NodeInfo>(inputNetwork.getNodeCount());
        TreeMap<Double,ArrayList> nodeScoreSortedMap = new TreeMap<Double,ArrayList>(new Comparator() { //will store Doubles (score) as the key, Lists as values
            //sort Doubles in descending order
            public int compare(Object o1, Object o2) {
                double d1 = ((Double) o1).doubleValue();
                double d2 = ((Double) o2).doubleValue();
...
//                          //store all the nodes that have already been seen and incorporated in other clusters
    ...
    private void calNodeInfos(CyNetwork net){
		HashMap<Integer,NodeInfo> nodeInfos=new HashMap();
    ...
    private void mergeComplexes1(Cluster c1,Cluster c2){
    	...
    	//System.out.println("Merge "+c1.getComplexID()+" to "+c2.getComplexID());
    	
    	ArrayList<Integer> alNodes=c1.getALNodes();
    ...
    private void mergeComplexes3(Cluster c1, Cluster c2){
    	ArrayList<Integer> nodes1=c1.getALNodes();
    	ArrayList<Integer> nodes2=c2.getALNodes();
        /*System.out.println("Merge "+c1.getComplexID()+" to "+c2.getComplexID());
    	for(Iterator i1=nodes1.iterator();i1.hasNext();){
    		int in=((Integer)i1.next()).intValue();
    		String name=currentNetwork.getNode(in).getIdentifier();
    	...
    	NodeInfo nodeNI;
    	ArrayList<Integer> subComplexes;
