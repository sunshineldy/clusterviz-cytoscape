...
/**
 * Show a Tabele Browser for the results of clustering. This class sets up the GUI.
 */
public class ResultPanel extends JPanel {
    ...
    /**
     * Constructor for the Results Panel which displays the complexes in a browswer table and
     ...
     * @param imageList A list of images of the found comple
    ...
    private JPanel createBrowserPanel(Image imageList[]) {
        ...
        //the radio buttons
        boolean set3=currentParamsCopy.getAlgorithm().equals(ParameterSet.KCORE);
        ...
        sortPanel.add(way2);
        sortPanel.add(way3);
        sortPanel.setToolTipText("选择一种方式对结果进行排序");
        
        panel.add(sortPanel,BorderLayout.NORTH);
        panel.add(tableScrollPane,BorderLayout.CENTER);
        panel.setToolTipText("识别出的模块的有关信息");
        return panel;
    }
    /**
    ...
    private JPanel createBottomPanel() {
        ...
        exportButton.setToolTipText("将结果导出为文本文件");
        JButton closeButton = new JButton("Discard");
        closeButton.setToolTipText("关闭本面板");
    ...
    private void switchPlace(String field){
    	...
    	Image image;
    	Cluster cluster;
    	System.out.println("list length "+complexes.length);
    ...
    private JPanel createExploreContent(int selectedRow) {
        ...
        nodeAttributes.setToolTipText("选择你想要查看的属性名");
        ...
        String sizeTip = "移动滑动块可以增大或减小模块尺寸";
        ...
        createChildButton.setToolTipText("为此模块创建一个网络图形");JPanel buttonPanel=new JPanel();
        buttonPanel.setLayout(new BorderLayout());
        buttonPanel.add(createChildButton,BorderLayout.SOUTH);
        JPanel bottom=new JPanel();
        bottom.setLayout(new BorderLayout());
        if(alg.getParams().getAlgorithm().equals(ParameterSet.KCORE))
    ...
    */
    ...
    private class DiscardAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            String message = "确定要关闭" + resultTitle + "吗?";
            int result = JOptionPane.showOptionDialog(Cytoscape.getDesktop(), new Object[]{message}, "确认", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null, null, null);
    ...
    private class ExportAction extends AbstractAction {
        public void actionPerformed(ActionEvent e) {
            File file = FileUtil.getFile("保存结果", FileUtil.SAVE, new CyFileFilter[]{});
            if (file != null) {
                String fileName = file.getAbsolutePath();
                String message = "是否只需导出基本信息?\n" +
                		"(Y/基本信息，N/完整信息)";
                int result = JOptionPane.showOptionDialog(Cytoscape.getDesktop(), new Object[]{message}, "导出方式", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null, null, null);
                if (result == JOptionPane.YES_OPTION) {
                    ClusterUtil.exportSimpleClusters(alg, complexes, network, fileName);
                	JOptionPane.showMessageDialog( Cytoscape.getDesktop(), "导出完成!"); 
                }
                else if(result==JOptionPane.NO_OPTION){
                	ClusterUtil.exportResults(alg, complexes, network, fileName);
                	JOptionPane.showMessageDialog( Cytoscape.getDesktop(), "已导出完整信息!"); 
                }
            }
        }
    }
    ...
    public void selectCluster(GraphPerspective gpCluster) {
        ...
        if (gpCluster != null) {
            if (networkView != null) {
                ...
                if(Cytoscape.getDesktop().getCytoPanel(SwingConstants.SOUTH).getState() == CytoPanelState.DOCK) {
                    Cytoscape.getDesktop().setFocus(networkView.getIdentifier());
                }
            } else {
                JOptionPane.showMessageDialog(Cytoscape.getDesktop(), 
                		"没有可供选择节点的视图.", "没有网络视图", JOptionPane.INFORMATION_MESSAGE);
            }
